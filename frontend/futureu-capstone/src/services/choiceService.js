...
/**
 * Service for handling choice-related API requests
 */
class ChoiceService {
  /**
  ...
  /**
   * Create a new choice
   * @param {Object} choice - The choice object to create
   * @returns {Promise<Object>} - Created choice data
   */
  async createChoice(choice) {
    try {
      const response = await apiClient.post('/choice/postChoice', choice);
      return response.data;
    } catch (error) {
   ...
   * @param {Object} choiceDetails - The new choice details
   * @returns {Promise<Object>} - Updated choice data
   */
  async updateChoice(choiceId, choiceDetails) {
    try {
      const response = await apiClient.put('/choice/putChoice', choiceDetails, {
        params: { choiceId }
      });
      return response.data;
    } catch (error) {
      this.handleError(error, 'Updating choice');
      throw error;
    }
  }
