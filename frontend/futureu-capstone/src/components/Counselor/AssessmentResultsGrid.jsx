      ...
      {groupByUser ? (
        <div className="flex flex-col gap-6 animate-fade-in">
          {Object.entries(grouped).map(([userKey, { user, attempts }]) => {
            ...
            if (!expanded) {
              // Only show the user card, no container background/border
              return (
                <div key={userKey} className="flex flex-col items-center pt-6 pb-3 px-6 min-w-[260px] max-w-[320px] w-full md:w-[320px] bg-gradient-to-r from-[#FFB71B]/10 to-[#1D63A1]/5 rounded-2xl shadow-lg animate-pop-in">
                  ...
                  <span
                    className="mt-2 ml-2 text-xs font-bold text-[#1D63A1] hover:text-[#FFB71B] underline cursor-pointer select-none"
            ...
            // Expanded: show user card and attempts in a row container
            return (
              <div key={userKey} className="relative flex flex-row items-stretch w-full  rounded-2xl shadow-lg hover:scale-[1.03] hover:shadow-2xl transition-all duration-300 group animate-pop-in overflow-hidden">
                {/* Header: Avatar and Name */}
                <div className="flex flex-col items-center justify-center px-6 py-4 min-w-[320px] max-w-[320px] self-stretch bg-gradient-to-r from-[#FFB71B]/10 to-[#1D63A1]/5">
                  <div className="w-16 h-16 rounded-full bg-gradient-to-br from-[#1D63A1] to-[#FFB71B] flex items-center justify-center text-white text-3xl font-extrabold shadow mb-2 animate-bounce-in">
                    {getInitials(user)}
                  </div>
                  <h3 className="font-bold text-xl text-[#1D63A1] mb-0.5 text-center tracking-tight">{user.firstName} {user.lastName}</h3>
                ...
                {/* Attempts list to the right, fully expanding */}
                <div className="flex-1 min-w-0 flex flex-row items-stretch overflow-x-auto gap-4 p-4 h-full">
                  {sorted.map((result, idx) => {
                    ...
                    return (
                      <div
                        ...
                        className={`bg-white rounded-lg border border-gray-200 p-4 flex flex-col items-center w-64 min-w-[240px] relative ${isMostRecent ? 'ring-2 ring-[#FFB71B]' : ''}`}
                      >
                        ...
                        <div className="flex flex-wrap gap-1 mb-2 justify-center">
                          <span className="px-2 py-1 rounded bg-emerald-100 text-emerald-700 text-xs font-semibold animate-badge">Overall: {typeof result.overallScore === 'number' ? result.overallScore.toFixed(2) : result.overallScore}</span>
                        ...
                        <button
                          className="mt-3 px-3 py-1 rounded-lg bg-gradient-to-r from-[#FFB71B] to-[#FFB71B] hover:from-[#2B3E4E] hover:to-[#2B3E4E] text-white text-xs font-bold transition-colors shadow"
