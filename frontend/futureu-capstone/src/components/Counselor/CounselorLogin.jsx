...
const CounselorLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Check if user has career guidance role (case-insensitive)
      const role = authService.getUserRole();

      if (!role || role.toUpperCase() !== 'CAREER_GUIDANCE') {
        setError('Access denied. Career Guidance privileges required.');
        ...
        <div className="p-10 md:p-12 relative z-10">
          <div className="flex items-center mb-10 justify-start">
            <div className="bg-gradient-to-br from-[#2B3E4E] to-[#2B3E4E]/90 p-4 rounded-2xl mr-5 shadow-lg">
              <Heart className="text-[#FFB71B] text-2xl" />
            </div>
            <div className="text-left">
              <h3 className="text-3xl font-bold text-[#2B3E4E] dark:text-blue-400">Counselor Portal</h3>
              <p className="text-gray-500 dark:text-gray-400 mt-1">Sign in with counselor credentials</p>
            </div>
          </div>
