...
public class ProfileController {
    ...
    public ResponseEntity<?> uploadProfilePicture(@PathVariable int userId, @RequestParam("file") MultipartFile file) {
        try {
            System.out.println("=== UPLOAD PICTURE REQUEST ===");
            System.out.println("UserId: " + userId);
            System.out.println("File: " + file.getOriginalFilename());
            
            String imageUrl = profileService.uploadProfilePicture(userId, file);
            
            System.out.println("Picture uploaded successfully: " + imageUrl);
            return ResponseEntity.ok(Map.of("imageUrl", imageUrl));
        } catch (IOException e) {
            System.err.println("IO Error uploading profile picture: " + e.getMessage());
            e.printStackTrace();
            return ResponseEntity.badRequest().body(Map.of("error", e.getMessage()));
        } catch (Exception e) {
    ...
    public ResponseEntity<?> deleteProfilePicture(@PathVariable int userId) {
        try {
            System.out.println("=== DELETE PICTURE REQUEST ===");
            System.out.println("UserId: " + userId);
