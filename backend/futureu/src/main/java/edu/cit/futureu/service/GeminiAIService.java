...
public class GeminiAIService {
    ...
    private Map<String, Double> identifyTopStrengthAreas(AssessmentResultEntity result) {
        ...
        // Check STEM aptitude
        if (result.getStemScore() != null && result.getStemScore() > 60) {
            strengths.put("STEM", result.getStemScore());
        }
        
        // Check ABM aptitude
        if (result.getAbmScore() != null && result.getAbmScore() > 60) {
            strengths.put("ABM", result.getAbmScore());
        }
        
        // Check HUMSS aptitude
        if (result.getHumssScore() != null && result.getHumssScore() > 60) {
            strengths.put("HUMSS", result.getHumssScore());
        }
        
        // Check Arts aptitude
        if (result.getArtsDesignTrackScore() != null && result.getArtsDesignTrackScore() > 60) {
            strengths.put("ARTS", result.getArtsDesignTrackScore());
        }
        
        // Check Sports aptitude
        if (result.getSportsTrackScore() != null && result.getSportsTrackScore() > 60) {
            strengths.put("SPORTS", result.getSportsTrackScore());
        }
        
        // Check TVL aptitude
        if (result.getTvlScore() != null && result.getTvlScore() > 60) {
            strengths.put("TVL", result.getTvlScore());
        }
        
        // If no clear strengths, identify based on individual abilities
        if (strengths.isEmpty()) {
            // Check math abilities
            if (result.getMathematicalAbilityScore() != null && result.getMathematicalAbilityScore() > 60) {
                strengths.put("STEM", result.getMathematicalAbilityScore());
            }
            
            // Check verbal abilities
            if (result.getVerbalAbilityScore() != null && result.getVerbalAbilityScore() > 60) {
                strengths.put("HUMSS", result.getVerbalAbilityScore());
            }
            
            // Add more fallbacks as needed
        }
        
        // If still no clear strengths, use RIASEC
        if (strengths.isEmpty()) {
            // Map RIASEC interests to program categories
            Map<String, Double> riasecToCategory = new HashMap<>();
            
            // Map Realistic to STEM/TVL
            if (result.getRealisticScore() != null && result.getRealisticScore() > 3) {
                riasecToCategory.put("STEM", (double)result.getRealisticScore());
                riasecToCategory.put("TVL", (double)result.getRealisticScore());
            }
            
            // Map Investigative to STEM
            if (result.getInvestigativeScore() != null && result.getInvestigativeScore() > 3) {
                riasecToCategory.put("STEM", (double)result.getInvestigativeScore());
            }
            
            // Map Artistic to ARTS and HUMSS
            if (result.getArtisticScore() != null && result.getArtisticScore() > 3) {
                riasecToCategory.put("ARTS", (double)result.getArtisticScore());
                riasecToCategory.put("HUMSS", (double)result.getArtisticScore());
            }
            
            // Map Social to HUMSS
            if (result.getSocialScore() != null && result.getSocialScore() > 3) {
                riasecToCategory.put("HUMSS", (double)result.getSocialScore());
            }
            
            // Map Enterprising to ABM
            if (result.getEnterprisingScore() != null && result.getEnterprisingScore() > 3) {
                riasecToCategory.put("ABM", (double)result.getEnterprisingScore());
            }
            
            // Map Conventional to ABM
            if (result.getConventionalScore() != null && result.getConventionalScore() > 3) {
                riasecToCategory.put("ABM", (double)result.getConventionalScore());
            }
            
            // Add top RIASEC-based strengths
            strengths.putAll(riasecToCategory);
        }
        
        // If still no clear strengths, provide default categories
        if (strengths.isEmpty()) {
